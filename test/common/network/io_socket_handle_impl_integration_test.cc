...
class IoSocketHandleImplPeekTest : public TcpListenerCallbacks, public testing::TestWithParam<Address::IpVersion> {
public:
  void initialize() {
...
TEST_P(IoSocketHandleImplPeekTest, error) {
  ...
  EXPECT_CALL(os_sys_calls, getsockopt_(_, _, _, _, _))
      ...
      .WillRepeatedly(Invoke(
          [&](os_fd_t sockfd, int level, int optname, void* optval, socklen_t* optlen) -> int {
            return os_sys_calls_actual.getsockopt(sockfd, level, optname, optval, optlen)
                .return_value_;
          }));
  EXPECT_CALL(os_sys_calls, getsockname(_, _, _))
      .Times(AnyNumber())
      .WillRepeatedly(Invoke(
          [&](os_fd_t sockfd, sockaddr* name, socklen_t* namelen) -> Api::SysCallIntResult {
  ...
  EXPECT_CALL(os_sys_calls, socket(_, _, _))
      .Times(AnyNumber())
      .WillRepeatedly(Invoke(
          [&](int domain, int type, int protocol) -> Api::SysCallSocketResult {
            return os_sys_calls_actual.socket(domain, type, protocol);
          }));
  EXPECT_CALL(os_sys_calls, setsocketblocking(_, _))
      .Times(AnyNumber())
      .WillRepeatedly(Invoke(
          [&](os_fd_t sockfd, bool blocking) -> Api::SysCallIntResult {
  ...
  EXPECT_CALL(os_sys_calls, listen(_, _))
      .Times(AnyNumber())
      .WillRepeatedly(Invoke(
          [&](os_fd_t sockfd, int backlog) -> Api::SysCallIntResult {
            return os_sys_calls_actual.listen(sockfd, backlog);
          }));
  EXPECT_CALL(os_sys_calls, setsockopt_(_, _, _, _, _))
      .Times(AnyNumber())
      .WillRepeatedly(Invoke(
          [&](os_fd_t sockfd, int level, int optname, const void* optval, socklen_t optlen) -> int {
