...
TEST_P(TlsInspectorTest, SniRegistered) {
  ...
  EXPECT_CALL(io_handle_, recv(_, _, MSG_PEEK))
      .WillOnce(Invoke(
          [&client_hello](void* buffer, size_t length, int) -> Api::IoCallUint64Result {
            ASSERT(length >= client_hello.size());
            memcpy(buffer, client_hello.data(), client_hello.size());
            return Api::IoCallUint64Result(client_hello.size(), Api::IoErrorPtr(nullptr, [](Api::IoError*) {}));
          }));
...
TEST_P(TlsInspectorTest, MultipleReads) {
      ...
      std::get<0>(GetParam()), std::get<1>(GetParam()), servername, "\x02h2");
  {
    ...
    for (size_t i = 1; i <= client_hello.size(); i++) {
      EXPECT_CALL(io_handle_, recv(_, _, MSG_PEEK))
          .WillOnce(Invoke([&client_hello, i](void* buffer, size_t length, int) -> Api::IoCallUint64Result {
            ASSERT(length >= client_hello.size());
            memcpy(buffer, client_hello.data(), client_hello.size());
...
TEST_P(TlsInspectorTest, ClientHelloTooBig) {
  ...
  EXPECT_CALL(io_handle_, close()).WillOnce(Return(ByMove(Api::IoCallUint64Result(0, Api::IoErrorPtr(nullptr, [](Api::IoError*) {})))));
