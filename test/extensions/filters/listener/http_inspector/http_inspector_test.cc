...
public:
...
#ifdef WIN32
      EXPECT_CALL(os_sys_calls_, readv(_, _, _)).WillOnce(
  ...
  void testHttpInspectSuccess(absl::string_view header, absl::string_view alpn) {
    ...
    if (alpn == Http::Utility::AlpnNames::get().Http2c) {
      EXPECT_CALL(os_sys_calls_, readv(_, _, _))
            .WillOnce(Invoke(
                  [&data](os_fd_t fd, const iovec* iov, int iovcnt) -> Api::SysCallSizeResult {
                  ASSERT(iov->iov_len >= data.size());
      ...
      EXPECT_CALL(os_sys_calls_, readv(_, _, _))
            .WillOnce(Invoke(
                  [&header](os_fd_t fd, const iovec* iov, int iovcnt) -> Api::SysCallSizeResult {
                  ASSERT(iov->iov_len >= header.size());
...
TEST_F(HttpInspectorTest, Http1WithLargeRequestLine) {
  ...
  const std::string data = absl::StrCat(method, spaces, http);
  {
    ...
    EXPECT_CALL(os_sys_calls_, readv(_, _, _))
        .Times(num_loops)
        .WillRepeatedly(Invoke([&data, ctr, copy_len, num_loops](os_fd_t fd, const iovec* iov, int iovcnt) -> Api::SysCallSizeResult {
