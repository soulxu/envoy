...
public:
  HttpInspectorTest() : cfg_(std::make_shared<Config>(store_)) {}
      ...
      if (http2) {
        for (size_t i = 1; i <= data.size(); i++) {
          EXPECT_CALL(io_handle_, recv(_, _, MSG_PEEK))
              .WillOnce(Invoke([&data, i](void* buffer, size_t length,
    ...
    if (alpn == Http::Utility::AlpnNames::get().Http2c) {
      EXPECT_CALL(io_handle_, recv(_, _, MSG_PEEK))
          .WillOnce(Invoke([&data](void* buffer, size_t length, int) -> Api::IoCallUint64Result {
            ...
            return Api::IoCallUint64Result(data.size(),
                                           Api::IoErrorPtr(nullptr, [](Api::IoError*) {}));
          }));
    } else {
      EXPECT_CALL(io_handle_, recv(_, _, MSG_PEEK))
          .WillOnce(Invoke([&header](void* buffer, size_t length, int) -> Api::IoCallUint64Result {
            ASSERT(length >= header.size());
            memcpy(buffer, header.data(), header.size());
            return Api::IoCallUint64Result(header.size(),
                                           Api::IoErrorPtr(nullptr, [](Api::IoError*) {}));
          }));
    }
    const std::vector<absl::string_view> alpn_protos{alpn};
  ...
  const std::string data = absl::StrCat(method, spaces, http);
  {
    ...
    EXPECT_CALL(io_handle_, recv(_, _, MSG_PEEK))
        ...
        .WillRepeatedly(Invoke(
            [&data, ctr, num_loops](void* buffer, size_t length, int) -> Api::IoCallUint64Result {
              size_t len = (*ctr);
              if (num_loops == 2) {
                ASSERT(*ctr != 3);
...
TEST_F(HttpInspectorTest, Http1WithLargeHeader) {
...

  {
    ...
    for (size_t i = 1; i <= 20; i++) {
      EXPECT_CALL(io_handle_, recv(_, _, MSG_PEEK))
          .WillOnce(Invoke([&data, i](void* buffer, size_t length, int) -> Api::IoCallUint64Result {
            ASSERT(length >= data.size());
            memcpy(buffer, data.data(), i);
            return Api::IoCallUint64Result(i, Api::IoErrorPtr(nullptr, [](Api::IoError*) {}));
          }));
