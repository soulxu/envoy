...
MockListenerConfig::MockListenerConfig()
  ...
  addresses_.emplace_back(socket_->connectionInfoProvider().localAddress());
  ...
  ON_CALL(socket_factory_, localAddresses()).WillByDefault(Return(addresses_));
  ON_CALL(socket_factory_, getListenSocket(_, _)).WillByDefault(Return(socket_));
