...
void ListenerManagerImpl::addListenerToWorker(Worker& worker,
  ...
  worker.addListener(overridden_listener, listener.perAddressConfig(),
                     [this, completion_callback]() -> void {
                       // The add listener completion runs on the worker thread. Post back to the
                       // main thread to avoid locking.
