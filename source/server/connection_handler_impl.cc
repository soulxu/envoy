...
void ConnectionHandlerImpl::addListener(absl::optional<uint64_t> overridden_listener,
  ...
  if (support_udp_in_place_filter_chain_update && overridden_listener.has_value()) {
    auto iter = listener_map_by_tag_.find(overridden_listener.value());
    ASSERT(iter != listener_map_by_tag_.end());
    for (auto& details : iter->second) {
      details->listener_->updateListenerConfig(config.perAddressConfig(0));
    }
    return;
  }

  auto details_list = std::vector<std::shared_ptr<ActiveListenerDetails>>();
  if (config.internalListenerConfig().has_value()) {
    auto details = std::make_shared<ActiveListenerDetails>();
    for (uint i = 0; i < config.listenSocketFactories().size(); i++) {
      if (overridden_listener.has_value()) {
        if (auto iter = listener_map_by_tag_.find(overridden_listener.value());
          ...
          for (uint j = 0; j < iter->second.size(); j++) {
            iter->second[j]->internalListener()->get().updateListenerConfig(
                config.perAddressConfig(j));
          }
          return;
        }
      ...
      auto internal_listener =
          std::make_unique<ActiveInternalListener>(*this, dispatcher(), config.perAddressConfig(i));
      ...
      details->address_ = config.listenSocketFactories()[i]->localAddress();
      // This map only store the new listener.
      internal_listener_map_by_address_.insert_or_assign(
          config.listenSocketFactories()[i]->localAddress()->asStringView(), details);
    }
    details_list.emplace_back(details);
    ...
    for (uint i = 0; i < config.listenSocketFactories().size(); i++) {
      if (!support_udp_in_place_filter_chain_update && overridden_listener.has_value()) {
        if (auto iter = listener_map_by_tag_.find(overridden_listener.value());
            iter != listener_map_by_tag_.end()) {
          for (uint j = 0; j < iter->second.size(); j++) {
            iter->second[j]->tcpListener()->get().updateListenerConfig(config.perAddressConfig(j));
          }
          return;
        }
        NOT_REACHED_GCOVR_EXCL_LINE;
      ...
      auto tcp_listener =
          std::make_unique<ActiveTcpListener>(*this, config.perAddressConfig(i),
                                              config.listenSocketFactories()[i]->getListenSocket(
    ...
    for (uint i = 0; i < config.listenSocketFactories().size(); i++) {
      ConnectionHandler::ActiveUdpListenerPtr udp_listener =
          config.udpListenerConfig()->listenerFactory().createActiveUdpListener(
              *worker_index_, *this,
              config.listenSocketFactories()[i]->getListenSocket(*worker_index_), dispatcher_,
              config.perAddressConfig(i));
  ...
  listener_map_by_tag_.emplace(config.listenerTag(), details_list);
}
...
void ConnectionHandlerImpl::removeListeners(uint64_t listener_tag) {
  if (auto listener_iter = listener_map_by_tag_.find(listener_tag);
    ...
    for (auto& details : listener_iter->second) {
      auto address_view = details->address_->asStringView();
      if (tcp_listener_map_by_address_.contains(address_view) &&
          tcp_listener_map_by_address_[address_view]->listener_tag_ == details->listener_tag_) {
...
Network::UdpListenerCallbacksOptRef ConnectionHandlerImpl::getUdpListenerCallbacks(
  ...
  if (auto listener_it = listener_map_by_tag_.find(listener_tag);
    ...
    for (auto& details : listener_it->second) {
...
Network::BalancedConnectionHandlerOptRef ConnectionHandlerImpl::getBalancedHandlerByTag(
    uint64_t listener_tag, const Network::Address::InstanceConstSharedPtr& address) {
  if (auto iter = listener_map_by_tag_.find(listener_tag); iter != listener_map_by_tag_.end()) {
    for (auto& details : iter->second) {
      if (*details->address_ == *address) {
